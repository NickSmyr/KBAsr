...
from google_kb_z_speaker.preprocessing import (
    get_swedish_phonemes_z,
    singular_phonemes_preprocess,
)
...
def sentence_lengths(bunches):
    return {
        k + "-avg-length": mean([len(x.split(" ")) for x in v])
...
def lcs_percentage(bunches):
    ...
    for c_l, g_l, kb_l in zip(bunches["correct"], bunches["google"], bunches["kb"]):
        ...
        lcs = "".join(
            [
                g_l_enc[block.a : (block.a + block.size)]
...
def parameterized_agreement(
    bunches,
    phonemizer_path,
    _filter=None,
    phoneme_words=None,
    singular_phonemes=None,
...
def meval(
    ...
    res = {
        **google_kb_wer(bunches),
        **sentence_lengths(bunches),
        **lcs_percentage(bunches),
    }
    if _filter is None:
...
def ensemble(
    ...
    bunches=None,
    use_phonemizer=None,
    save_ensemble_output=None,
):
    ensemble_outputs = get_ensemble_output(
        bunches, phonemizer, ratio_threshold, use_phonemizer
    )
    res = {
        **google_kb_wer(bunches),
        "ensemble-wer": wer(bunches["correct"], ensemble_outputs),
    }
...
def get_ensemble_output(
    bunches: Union[Dict[str, str], Dict[str, List[str]]],
    phonemizer,
    ...
    block_list_unmatched_idxs_tuple_list: List[
        ...
        for ii, pp in enumerate(block_list):
            ...
            if ii in unmatched_idxs:
                ...
                locator2unmatched_block_pair_index[(i, ii)] = (
                    len(unmatched_blocks) - 2,
                    len(unmatched_blocks) - 1,
                )
    ...
    ensemble_output = []
    for i, block_list_unmatched_idxs_tuple in enumerate(
        block_list_unmatched_idxs_tuple_list
    ):
            ...
            if ii in unmatched_idxs:
                if use_phonemizer:
                    ...
                    ratio = SequenceMatcher(
                        None,
                        phonemized_block_pair[0],
                        phonemized_block_pair[1],
                    ).ratio()
                else:
