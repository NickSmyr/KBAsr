...
for i in range(len(sps)):
    wav_temp = wav_out[
        int(sr * tg_start[sps[i] - 1]) : int(sr * tg_stop[sps[i] + 1])
    ]
    wav_name = f"{episode}_{str(i).zfill(4)}.wav"
    soundfile.write(
        output_loc + "/temp/" + wav_name, wav_temp, sr, subtype="PCM_16"
    )

asr_files = list(
    list_filenames(output_loc + "/temp", extensions=".wav", add_ext=False)
)
asr_files.sort()
...
] = "./codes/transcribespeech-312319-d025b453aaeb.json"

#%% transcribe list
results = [None] * len(sps)
longresults = [None] * len(sps)
client_ops = client_options.ClientOptions(
...
for i in range(len(sps)):
    ...
    with io.open(
        f"{output_loc}/temp/{episode}_{str(i).zfill(4)}.wav", "rb"
    ) as audio_file:
...
for k in range(len(transcript)):
    if transcript[k]:
        ...
        transcript2[k] = re.sub(
            r"([a-z])\-([a-z])", r"\1 \2", transcript2[k], 0, re.IGNORECASE
        )
...
annotTier = tgio.IntervalTier(
    "labels", [], 0, pairedWav=orig_wav_root + infiles[epi] + ".wav"
)
asrTier = tgio.IntervalTier(
    "transcript", [], 0, pairedWav=orig_wav_root + infiles[epi] + ".wav"
)
...
for i in range(len(tg_start)):
    annotTier.insertEntry(
        (tg_start[i], tg_stop[i], tg_label[i]),
        warnFlag=True,
        collisionCode="replace",
    )
    asrTier.insertEntry(
        (tg_start[i], tg_stop[i], tg_asr[i]),
        warnFlag=True,
        collisionCode="replace",
    )
