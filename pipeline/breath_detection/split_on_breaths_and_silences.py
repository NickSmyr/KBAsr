...
def split_on_breaths_and_silences(
    ...
    output_root = (
        output_directory  # location of other outputs from the process
    )
    trained_model = (
        breath_detection_model  # pretrained model for breath identification
    )
    ...
    n_mels = 128
    timesteps = (
        40  # timesteps in keras model, standard 2 seconds with 20 steps each
    )
    slicepersec = 400
    ...
    try:
        ...
        flat_pred = np.load(
            join(output_root, "predictions/", output_prefix + ".npy")
        )
    ...
    w_id = pred[
        w_change
    ]  # identify changes in segment (breath, speech, speaker)
    w_br = np.where(np.logical_or(w_id == 1, w_id == 2))
    w_br = np.where((w_id == 1) | (w_id == 2))[
        0
    ]  # identify breaths 2 speakers
        ...
        if output[i, 0] == -1:
            ...
            if (
                ...
                if len(ends) > 0:
                    ...
                    output[i, 5] = output[
                        i, 4
                    ]  # speech time before shortening
                    output[i, 4] = ends[-1]
                    if len(np.where(ends[:-1] != (ends[1:] - 1))[0]) > 0:
                        output[i, 6] = (
                            len(ends)
                            - np.where(ends[:-1] != (ends[1:] - 1))[0][-1]
                            - 1
                    ...
                    output[i, 5] = output[
                        i, 4
                    ]  # speech time of too short utterance
                    output[i, 6] = (
                        output[i, 2]
                        - output[i, 1]
                        - output[i, 4]
