...
def colorvec(
    ...
    z = zcr2 / maxzcr
    z[z > 1] = 1
    low = np.outer(low_slow, np.ones_like(z)) + np.outer(
        low_fast - low_slow, z
    )
    high = np.outer(high_slow, np.ones_like(z)) + np.outer(
        high_fast - high_slow, z
    )
    for k in range(3):
        outp[:, :, k] = np.tile(
            low[k, :], (np.shape(spec2)[0], 1)
        ) + spec2 * np.tile(high[k, :] - low[k, :], (np.shape(spec2)[0], 1))
    outp = outp / 255
    return outp
...
def annot2textgrid(filename, labels, annot, timesteps=40):
    ...
    annotTier.insertEntry(
        (0, w_change[0] / fps, labels[annot[0]]),
        warnFlag=True,
...
def annot_txt2textgrid(
    ...
    input = pd.read_csv(
        filename + ".txt", sep="\t", header=None, encoding="UTF-8"
    )
    ...
    for i in range(len(input)):
        if input[4][i] != input[4][i]:
            if input[3][i].strip() in annot_labels:
                ...
                textTier.insertEntry(
                    (input[1][i], input[2][i], input[3][i]),
                    warnFlag=True,
                    collisionCode="replace",
                )
                phonemeTier.insertEntry(
                    (input[1][i], input[2][i], ""),
                    warnFlag=True,
                    collisionCode="replace",
                )
                ...
                print(
                    f"incorrect entry (missing phonemic transcript or incorrect label) in entry {i}"
                )
        else:
            annotTier.insertEntry(
                (input[1][i], input[2][i], labels[annot_labels.index("")]),
                warnFlag=True,
                collisionCode="replace",
            )
            textTier.insertEntry(
