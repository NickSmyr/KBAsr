...
sb = [
    i for i, x in enumerate(tg_label) if x in ["sil", "b"]
]  # silence and breaths
...
df = pd.DataFrame.from_records(
    zip(outfiles, sps), columns=["file", "location"]
)
df["startloc"] = 0
df["starttime"] = 0.0
df["endloc"] = 0
...
for i in range(len(df)):
    ...
    df["endloc"][i] = sps[
        next((i for i, x in enumerate(speechend) if x > maxtime), 0) - 1
    ]
...
for i, row in df_out.iterrows():
    wwrite = wav_out[
        int(row["starttime"] * sr) : int((row["endtime"] + margin) * sr)
    ]
    wwrite = np.concatenate(
        (np.zeros(int(trail * sr)), wwrite, np.zeros(int(trail * sr)))
    )
    soundfile.write(
        output_loc + row["file"] + ".wav", wwrite, sr, subtype="PCM_16"
    )
