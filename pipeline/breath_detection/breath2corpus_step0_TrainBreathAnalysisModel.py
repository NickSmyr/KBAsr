...
def transcript2annot(
    filename, labels, timesteps=40, other=0, merge_breath=False
):
    fps = timesteps / 2
    ...
    if merge_breath:  # [n, m] if n preceded of followed by m, merge with m
        for i in range(1, len(num_label) - 1):
            if num_label[i] == merge_breath[0] and (
                num_label[i + 1] == merge_breath[1]
                or num_label[i - 1] == merge_breath[1]
            ):
...
for i in range(5, 25):
    ...
    annot = np.append(
        annot, np.reshape(temp[: timesteps * adds], (adds, timesteps)), axis=0
    )
    ...
    col_in = [(melspecs[r], zrates[r]) for r in range(adds)]
...
a_mask = np.any(np.greater(annot, 2.0), axis=1)
annot = annot[~a_mask, :]
specs = specs[~a_mask, :, :, :]
...
indices = random.sample(
    range(1, np.shape(specs)[0]), min(5, np.shape(specs)[0])
)
for i in indices:
...
files2 = sorted(
    list(codes.helpers.list_filenames(output_root + "zcrgrams", [".png"]))
)
im = cv2.imread(files2[0])
x_complete = np.empty(
    (np.shape(files2)[0], np.shape(im)[0], np.shape(im)[1], np.shape(im)[2])
)
...
for j in range(0, np.shape(x2_train)[0]):
    for k in range(0, timesteps):
        x2_train[j, k, :, :, :] = x_train[
            j, :, k * img_cols : (k + 1) * img_cols, :
        ]
...
modelZM1.add(
    TimeDistributed(
        Conv2D(8, kernel_size=(4, 1), strides=(4, 1), activation="relu")
    )
)
modelZM1.add(TimeDistributed(BatchNormalization()))
...
train_ind = np.where(
    np.sum(y_train[:, :, 0], axis=1) + np.sum(y_train[:, :, 2], axis=1) > 0
)[0]
test_ind = np.where(
    np.sum(y_test[:, :, 0], axis=1) + np.sum(y_test[:, :, 2], axis=1) > 0
)[0]
