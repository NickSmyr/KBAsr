...
def extract_speaker_embedding_2(signal, audio):
    audio_file = torch.tensor(
        audio[signal.start * 1000: signal.end * 1000].get_array_of_samples()
    )
    # embeddings = classifier.encode_batch(audio_file)
    embeddings = inference(
        {"waveform": audio_file.unsqueeze(0).float(), "sample_rate": 44100}
    )
...
def create_speaker_files_from_audio_path(audio_path):
    ...
    in_memory_audio = {"waveform": wave, "sample_rate": sr}

    # apply pretrained pipeline
    audio = AudioSegment.from_wav(audio_path)

    print("Running the diarization pipeline on ", audio_path)
    ...
    for turn, _, speaker in diarization.itertracks(yield_label=True):
        ...
        print("Speaker 1 len ", len(speaker1))
        print("Speaker 2 len ", len(speaker2))
...
def create_speaker_files_from_audio_path_old(audio_path):
    """
    Creates speaker files from the audio file at audio_path by splitting
    based on speaker.
...
def time_format(t):
    ...
    return ":".join([str(x) for x in [hours, minutes, seconds, fraction]])
