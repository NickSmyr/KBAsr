...
def split_on_breaths_and_silences(input_file_name, output_directory, breath_detection_model):
    # Vars
    # TODO remove unncesessary prints
    # TODO investigate WAV file Waring chunk non data not understood, skipping it
    ...
    output_prefix = 'C' + os.path.basename(input_file)  # naming convention for output files
    ...
    for n in range(0, np.shape(x_complete)[0]):
        ...
        fname = join(output_root, 'zcrgrams', output_prefix , 'zcr' + '{:04d}'.format(n) + '.png')
        img.save(fname, 'PNG')

    soundfile.write(join(data_root , output_prefix + '.wav'), y[1], y[3], subtype='PCM_16')
            ...
            if output[i, 4] > max_utt * fps:  # change end frame to last silence if longer than maximum length
                ...
                if len(ends) > 0:
                    ...
                    if len(np.where(ends[:-1] != (ends[1:] - 1))[0]) > 0:
                        output[i, 6] = len(ends) - np.where(ends[:-1] != (ends[1:] - 1))[0][
                            -1] - 1  # closing silence in frames
                    else:
                        output[i, 6] = len(ends)
                else:
                    output[i, 0] = 0
                    output[i, 5] = output[i, 4]  # speech time of too short utterance
                    output[i, 6] = output[i, 2] - output[i, 1] - output[i, 4] - output[
                        i, 3]  # closing breath in frames
            else:
    ...
    np.save(join(output_root, output_prefix, 'output_' + output_prefix + '.npy'), output)
    np.savetxt(join(output_root, output_prefix, 'output_' + output_prefix + '.csv'), output, delimiter=",")
